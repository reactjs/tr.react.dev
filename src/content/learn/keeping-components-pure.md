...
Eğer <Math><MathI>x</MathI> = 3</Math> ise, <MathI>y</MathI> günün saatine veya stock market’in durumuna bağlı olarak bazen <Math>9</Math>, <Math>–1</Math> veya <Math>2.5</Math> olmaz.
...
* Bir component pure olmalıdır, yani:
  * **Kendi işine bakar.** Rendering’den önce var olan hiçbir object veya variable’ı değiştirmemelidir.
  * **Aynı input’lar, aynı output.** Aynı input’lar verildiğinde, bir component her zaman aynı JSX’i return etmelidir.
* Rendering herhangi bir zamanda gerçekleşebilir, bu yüzden component’ler birbirlerinin rendering sequence’ine depend etmemelidir.
* Component’lerinizin rendering için kullandığı input’lardan hiçbirini mutate etmemelisiniz. Buna props, state ve context dahildir. Screen’i update etmek için, mevcut object’leri mutate etmek yerine ["set" state](/learn/state-a-components-memory) kullanın.
...
* Pure function yazmak biraz practice gerektirir, ancak React’in paradigm’ının gücünü ortaya çıkarır.
