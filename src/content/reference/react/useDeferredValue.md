...
1. **Öncelikle, React yeni `query` (`"ab"`) ile yeniden render yapar fakat eski `deferredQuery` (`"a"`) ile kalır.** Sonuç listesine ilettiğiniz `deferredQuery` değeri *ertelemlidir:* yani `query` değerinin gerisinde kalır.
