...
`<Fragment>`, genellikle `<>...</>` syntax’i üzerinden kullanılır ve element’leri wrapper node olmadan group etmenizi sağlar.
...
- `compareDocumentPosition(otherNode)`: Fragment’in document position’ını başka bir node ile compare eder.
  ...
  - Empty Fragment’ler, React tree içindeki positioning’i compare etmeye çalışır ve `Node.DOCUMENT_POSITION_IMPLEMENTATION_SPECIFIC` içerir.
