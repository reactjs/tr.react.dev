...
Değer olarak `false` bir ifade döndürmek, React'in `&&` ifadesinden sonra gelen kısmı atlamasına neden olacaktır. Ancak yine de o `falsy` değeri dönüp render edeceğini unutmayın. Aşağıdaki örnekte `render` metodundan `<div>0</div>` değeri dönecektir:
